...
const TodoEdit = ({editTodoItem, setEditTodoItem, todo, setTodo}
    ...
    return (
        <div className="todo-list__edit-item">
            {(editTodoItem.id === 0) ?
                    <h2 className="edit-item__title">Для редактирования задачи нажмите  <img src={require('../../assets/edit.png')} alt="edit_task"/></h2>
                    :
                <>
                    <h2 className="edit-item__title">Редактирование задачи №{editTodoItem.id}</h2>
                    <form>
                        <select className="task__status" value={status}
